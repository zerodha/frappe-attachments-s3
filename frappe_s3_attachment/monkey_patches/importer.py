...
class Importer_new:
	def __init__(
		...
		self.import_file = ImportFile_new(
			doctype,
...
class ImportFile_new:
	def __init__(self, doctype, file, template_options=None, import_type=None,import_id=None):
		...
		self.file_doc = self.file_path = self.google_sheets_url = self.s3_file = None
		file_doc = frappe.get_all("File",filters={"attached_to_name":import_id,"attached_to_doctype":"Data Import"},fields=["*"],order_by="creation desc")
		if len(file_doc) >= 1:
		...
		if not self.file_doc and not self.file_path and not self.google_sheets_url and not self.s3_file:
	...
	def get_data_from_template_file(self):
		...
		if self.s3_file:
			s3_path = generate_signed_url(self.s3_file.content_hash,self.s3_file.file_name)
			results = requests.get(s3_path)
			parts = self.s3_file.get_extension()
			extension = parts[1]
...
Importer.__init__ = Importer_new.__init__
ImportFile.__init__ = ImportFile_new.__init__